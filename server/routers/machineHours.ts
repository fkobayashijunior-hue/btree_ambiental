...
export const machineHoursRouter = router({
  ...
  createMaintenance: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Auto-generate financial entry for maintenance cost
      if (input.totalCost && parseFloat(input.totalCost.replace(',', '.')) > 0) {
        try {
          ...
          const typeLabel = input.type === 'preventiva' ? 'Preventiva' : input.type === 'corretiva' ? 'Corretiva' : 'Revisão';
          await db.insert(financialEntries).values({
            type: 'despesa',
            category: 'manutencao',
            description: `Manutenção ${typeLabel} - ${eqName}${input.description ? ': ' + input.description.slice(0, 80) : ''}`,
  ...
  createFuel: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Auto-generate financial entry for fuel cost
  ...
  equipmentSummary: protectedProcedure.query(async () => {
    ...
    const oilRecords = await db.select().from(equipmentOilRecords).orderBy(desc(equipmentOilRecords.createdAt));
    ...
    return equipmentList.map((eq: any) => {
      ...
      const eqOil = oilRecords.filter((o: any) => o.equipmentId === eq.id);
      ...
      const totalMaintCost = eqMaint.reduce((sum: number, m: any) => sum + (parseFloat(m.totalCost || "0") || 0), 0);
      const totalOilLiters = eqOil.reduce((sum: number, o: any) => sum + (parseFloat(o.quantityLiters) || 0), 0);
      const totalOilCost = eqOil.reduce((sum: number, o: any) => sum + (parseFloat(o.totalValue || "0") || 0), 0);
  ...
  // === ÓLEO ===
  listOil: protectedProcedure
    .input(z.object({ equipmentId: z.number().optional() }).optional())
    .query(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Banco indisponível" });
      const results = await db.select().from(equipmentOilRecords).orderBy(desc(equipmentOilRecords.createdAt));
      return input?.equipmentId ? results.filter((r: any) => r.equipmentId === input.equipmentId) : results;
    }),

  createOil: protectedProcedure
    .input(z.object({
      equipmentId: z.number(),
      date: z.string(),
      hourMeter: z.string().optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Auto-generate financial entry for oil cost
      if (input.totalValue && parseFloat(input.totalValue.replace(',', '.')) > 0) {
        try {
          const [eqRow] = await db.select({ name: equipment.name }).from(equipment).where(eq(equipment.id, input.equipmentId));
          const eqName = eqRow?.name || `Equipamento #${input.equipmentId}`;
          ...
          await db.insert(financialEntries).values({
            type: 'despesa',
            category: 'manutencao',
            description: `Óleo ${oilLabels[input.oilType] || input.oilType} - ${eqName} - ${input.quantityLiters}L${input.brand ? ' (' + input.brand + ')' : ''}`,
            amount: input.totalValue.replace(',', '.'),
            date: dateObj.toISOString().slice(0, 10),
            referenceMonth: refMonth,
            paymentMethod: 'transferencia',
  ...
  updateOil: protectedProcedure
    .input(z.object({
      id: z.number(),
      date: z.string().optional(),
      hourMeter: z.string().optional().nullable(),
      oilType: z.enum(["hidraulico", "motor", "transmissao", "diferencial", "outros"]).optional(),
      quantityLiters: z.string().optional(),
      brand: z.string().optional().nullable(),
      supplier: z.string().optional().nullable(),
      pricePerLiter: z.string().optional().nullable(),
      totalValue: z.string().optional().nullable(),
      notes: z.string().optional().nullable(),
    }))
    .mutation(async ({ input }) => {
      const db = await getDb();
      ...
      await db.update(equipmentOilRecords).set({
        ...rest,
        ...(date ? { date } : {}),
      }).where(eq(equipmentOilRecords.id, id));
      return { success: true };
    }),

  deleteOil: protectedProcedure
    .input(z.object({ id: z.number() }))
    .mutation(async ({ ctx, input }) => {
      if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
      const db = await getDb();
