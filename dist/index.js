    ...
    if (input.received) {
      ...
      if (existing.length === 0) {
        ...
        if (cargo && cargo.destinationId) {
          const [dest] = await db.select().from(cargoDestinations).where(eq6(cargoDestinations.id, cargo.destinationId)).limit(1);
          if (dest) {
...
var invoiceControlRouter = router({
  ...
  list: protectedProcedure.input(z34.object({
    ...
    limit: z34.number().optional().default(100)
  }).optional()).query(async ({ input }) => {
    const db = await getDb();
    ...
    const results = await db.select({
      ...
      clientNameJoined: clients.name
    }).from(cargoLoads).leftJoin(cargoDestinations, eq33(cargoLoads.destinationId, cargoDestinations.id)).leftJoin(clients, eq33(cargoLoads.clientId, clients.id)).where(eq33(cargoLoads.status, "entregue")).orderBy(desc28(cargoLoads.date)).limit(input?.limit ?? 100);
  ...
  toggleChecked: protectedProcedure.input(z34.object({ id: z34.number(), checked: z34.boolean() })).mutation(async ({ input, ctx }) => {
    const db = await getDb();
    if (!db) throw new TRPCError24({ code: "INTERNAL_SERVER_ERROR", message: "Banco indispon\xEDvel" });
    await db.update(cargoLoads).set({
  ...
  stats: protectedProcedure.query(async () => {
    ...
    const all = await db.select({
      invoiceChecked: cargoLoads.invoiceChecked,
      invoiceNumber: cargoLoads.invoiceNumber
    }).from(cargoLoads).where(eq33(cargoLoads.status, "entregue"));
